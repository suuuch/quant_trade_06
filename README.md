...
- 多头：MA20 或 MA30 过去 10 天平均每天上涨大于 0.3%；
- 空头：MA20 或 MA30 过去 10 天平均每天下跌大于 0.3%。

MA20/MA30 日均涨跌幅各自取最近 10 个交易日收益率的算术平均，日收益率按
`(MA_t - MA_{t-1}) / MA_{t-1}` 计算；满足其一即可。A 股与美股使用同一套条件。

美股扫描前还会限制股票代码以 `US.` 开头、市值大于 10 亿美元、最新
...
- `发送美股`：读取数据库中美股最新日线日期，发送这一天的美股 RSI 顺势
  交易结果；若没有缓存，会先扫描、生成图片、写缓存，再发送；
- `发送美股2026-07-25`：发送指定日期的美股 RSI 顺势交易结果。发送前会
  检查 `public.stock_klines` 是否有该日期数据；
